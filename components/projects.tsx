...
export function Projects() {
  ...
  return (
    <section id="projects" className="section-shell py-16 md:py-20">
      <Reveal>
      ...
      <div className="mt-7 grid gap-4 lg:grid-cols-3">
        {featuredProjects.map((project, index) => (
          <Reveal key={project.name} delay={0.12 + index * 0.08}>
            <article className="glass-project-card group rounded-3xl p-5 transition-all duration-300 md:p-6">
              <div className="mb-4 flex items-center justify-between">
                <span className="project-pill rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-accent/95">
                  {t("featured")}
                </span>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {project.tags.map((tag) => (
                  <span key={tag} className="project-tag rounded-full border border-white/12 px-2.5 py-1 text-xs text-foreground/90">
                    {tag}
                  </span>
                ))}
              </div>
